...
# =====================================================================
# 2. 新增与解耦模块：静态 CNN、动态 Swin、以及 AdaLN
# =====================================================================

class StaticEncoder2D(nn.Module):
    """ 处理 2D 地貌/植被变量的编码器 """
    def __init__(self, in_channels, out_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, out_dim, kernel_size=3, padding=1, stride=2),
...
class DynamicSwin3D(nn.Module):
    """ 纯净版 3D Swin，只负责动态气象提取 """
    def __init__(self, patch_size=(1, 1, 1), in_chans=10, embed_dim=32, depths=[2, 2, 8], num_heads=[2, 4, 8], window_size=(3, 5, 5), norm_layer=nn.LayerNorm):
        super().__init__()
...
class AdaLNFusion2D(nn.Module):
    """
    空间自适应层归一化 (Spatial AdaLN)
    将静态 2D 地貌特征转化为缩放(Scale)和偏移(Shift)，像素级物理调制 3D 天气特征。
    """
    def __init__(self, static_dim, dynamic_dim):
        super().__init__()
        # 使用 GroupNorm(1) 等价于在空间张量 [B, C, H, W] 上做 Channel 维度的 LayerNorm
        self.norm = nn.GroupNorm(1, dynamic_dim)
        
        # 使用 1x1 卷积代替 MLP，完美适配空间维度的逐像素运算
        self.modulator = nn.Sequential(
            nn.SiLU(),
            nn.Conv2d(static_dim, dynamic_dim * 2, kernel_size=1) # 输出 gamma 和 beta
        )
        
        # ================= 核心绝杀：零初始化 =================
        # 保证训练初期退化为普通的 Norm，使得梯度极度稳定，不会因为引入地形分支导致崩溃
        nn.init.constant_(self.modulator[1].weight, 0)
        nn.init.constant_(self.modulator[1].bias, 0)

    def forward(self, x_dynamic, x_static):
        # 1. 动态气象基础归一化
        x_norm = self.norm(x_dynamic)
        
        # 2. 静态地貌生成调制参数
        scale_shift = self.modulator(x_static)
        gamma, beta = scale_shift.chunk(2, dim=1) # 切分为 [B, C, H, W]
        
        # 3. 施加物理调制: (1 + 缩放系数) * 气象 + 偏移基础值
        return x_norm * (1 + gamma) + beta


# =====================================================================
# 3. 最终顶层 Wrapper：AdaLN + 3x3 ROI
# =====================================================================

class DualBranchSwinAdaLN(nn.Module):
    """ 
    终极版：AdaLN 物理调制 + 3x3 ROI 核心感知区的双分支架构
    """
    def __init__(self, input_channels_d=10, input_channels_s=5, input_channels_c=10, n_classes=2, **kwargs):
        ...
        # 2. 静态地形提取器 (2D)
        self.branch_static = StaticEncoder2D(in_channels=static_chans, out_dim=self.embed_dim)
        
        # 3. 替换为 AdaLN 物理融合模块
        self.adaln_fusion = AdaLNFusion2D(static_dim=self.embed_dim, dynamic_dim=self.embed_dim)
    ...
    def forward(self, data_s, data_c, data_d, data_t=None):
        ...
        # ================= 物理调制融合 (严格空间对齐) =================
        # [B, C, H_out, W_out]
        x_fused = self.adaln_fusion(x_dynamic=x_dyn_2d, x_static=x_stat_2d)
        
        # ================= 3x3 核心威胁区截取 (保留 Recall) =================
        center_h, center_w = H_out // 2, W_out // 2
        
        # 涵盖周围 8 个方向的逼近火情
        x_roi = x_fused[:, :, center_h-1 : center_h+2, center_w-1 : center_w+2] # [B, C, 3, 3]
        
        # 展平核心区
        x_roi_flat = x_roi.reshape(B, -1) # [B, C * 9]
        # ====================================================================
...
if __name__ == '__main__':
    ...
    # 实例化模型 (与 train.py 严格一致)
    model = DualBranchSwinAdaLN(input_channels_d=10, input_channels_s=5, input_channels_c=10, n_classes=2).to(device)
