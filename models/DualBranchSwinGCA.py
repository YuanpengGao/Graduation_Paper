...
class StaticEncoder2D(nn.Module):
    ...
    def forward(self, x_stat, target_size):
        # x_stat 已经是 data_s 和 data_c 拼接好的 [B, C_stat, H, W]
        x = self.net(x_stat)
        # 强制将空间分辨率与 3D 分支的输出对齐
...
class DualBranchSwinGCA(nn.Module):
    """ 
...
if __name__ == '__main__':
    ...
    test_d = torch.randn((4, 11, 10, 25, 25)).to(device)
    test_s = torch.randn((4, 5, 25, 25)).to(device)
    test_c = torch.randn((4, 10, 25, 25)).to(device)

    # 实例化双分支 GCA 模型
    model = DualBranchSwinGCA(
        input_channels_s=5, 
        input_channels_c=10, 
        input_channels_d=11,  # 动态通道必须指明是 11
