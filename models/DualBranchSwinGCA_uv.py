...
class DualBranchSwinGCA_uv(nn.Module):
    """ 
    采用 3x3 核心 ROI 感知区的双分支架构 (完美平衡 Precision 与 Recall)
    """
    def __init__(self, input_channels_d=10, input_channels_s=5, input_channels_c=10, n_classes=2, wind_u_idx=None, wind_v_idx=None, **kwargs):
        super().__init__()
    ...
    def forward(self, data_s, data_c, data_d, data_t=None):
        ...
        # 【新增】：拦截并保存分支独立的特征图供可视化！
        self.saved_x_dyn_2d = x_dyn_2d.detach().cpu()
        self.saved_x_stat_2d = x_stat_2d.detach().cpu()
        # =======================================================
        # 2. 用整数索引直接去切片张量！极其优雅且不报错
        # =======================================================
        wind_u, wind_v = None, None
        if self.wind_u_idx is not None and self.wind_v_idx is not None:
            # 直接用索引从 data_d 中拿出 U 和 V
            wind_u_raw = data_d[:, self.wind_u_idx, -1, :, :] 
            wind_v_raw = data_d[:, self.wind_v_idx, -1, :, :]
            wind_u = F.adaptive_avg_pool2d(wind_u_raw.unsqueeze(1), (H_out, W_out)).squeeze(1)
            wind_v = F.adaptive_avg_pool2d(wind_v_raw.unsqueeze(1), (H_out, W_out)).squeeze(1)

        # [B, C, H_out, W_out]
        x_fused = self.geo_ca(q_x=x_dyn_2d, kv_x=x_stat_2d, wind_u=wind_u, wind_v=wind_v)
        
        # 【新增】：拦截并保存融合后的特征图！
        self.saved_x_fused = x_fused.detach().cpu()
        '''
        # SwinGCA_2 
        # ================= 核心绝杀：3x3 核心威胁区截取 =================
...
if __name__ == '__main__':
    ...
    test_d = torch.randn((4, 12, 10, 25, 25)).to(device)
    test_s = torch.randn((4, 5, 25, 25)).to(device)
    test_c = torch.randn((4, 10, 25, 25)).to(device)

    # 实例化双分支 GCA 模型
    model = DualBranchSwinGCA_uv(
        input_channels_s=5, 
        input_channels_c=10, 
        input_channels_d=12,  # 动态通道必须指明是 11
        wind_u_idx=10,  # <--- 直接传数字
        wind_v_idx=11,
