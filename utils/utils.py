...
class evaluator():
    ...
    def get_results(self, mean_loss, best_loss):
        ...
        message += '\n%s Precision       : %.4f' % (self.mode, np.mean(self.precision[1]))
        message += '\n%s Recall          : %.4f' % (self.mode, np.mean(self.accuracy[1]))
        message += '\n%s F1-Score        : %.4f' % (self.mode, np.mean(self.F1[1]))
        message += '\n%s AUROC           : %.4f' % (self.mode, self.AUROC)
        message += '\n%s OA              : %.4f' % (self.mode, self.accuracy_all)
