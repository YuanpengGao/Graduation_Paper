...
class FireCubeLoader(Dataset):
    ...
    def _calculate_vpd(self, t2m_k, d2m_k):
        """
        基于 Magnus-Tetens 公式计算 VPD
        输入：t2m_k (开尔文温度), d2m_k (开尔文露点温度)
        输出：VPD (kPa)
        """
        # 1. 开尔文转摄氏度
        t2m_c = t2m_k - 273.15
        ...
        # 2. 计算饱和水汽压 es 和实际水汽压 ea (kPa)
        es = 0.611 * np.exp((17.27 * t2m_c) / (t2m_c + 237.3))
        ea = 0.611 * np.exp((17.27 * d2m_c) / (d2m_c + 237.3))
        
        # 3. 计算 VPD，并避免因为浮点数精度出现极小的负数
        vpd = es - ea
    ...
    def __getitem__(self, index):
        ...
        # ==========================================
        # 【物理先验注入】：在归一化前，利用原始物理量计算 VPD
        # ==========================================
        has_vpd_vars = ('era5_max_t2m' in self.dynamic_features) and ('era5_max_d2m' in self.dynamic_features)
        if has_vpd_vars:
            t2m_idx = self.dynamic_features.index('era5_max_t2m')
            d2m_idx = self.dynamic_features.index('era5_max_d2m')
            
            raw_t2m = data_dynamic[t2m_idx]
            raw_d2m = data_dynamic[d2m_idx]
            
            # 计算出 VPD 矩阵，形状为 (lag, H, W)
            vpd_matrix = self._calculate_vpd(raw_t2m, raw_d2m)
            
            # 物理规则缩放：地球上极端 VPD 一般不超过 8 kPa。
            # 我们直接用固定的物理上限 8.0 来做 MinMax 归一化到 [0, 1]，不需要读取 JSON
            vpd_scaled = np.clip(vpd_matrix / 8.0, 0.0, 1.0)
        # ==========================================
        ...
        # ==========================================
        # 【拼接 VPD】：将算好且缩放完毕的 VPD 拼接到 data_dynamic 中
        # ==========================================
        if has_vpd_vars:
            # 此时 vpd_scaled 是 (10, 25, 25)，需要扩增一个特征维度变成 (1, 10, 25, 25)
            vpd_scaled = np.expand_dims(vpd_scaled, axis=0)
            # 拼接到 dynamic_features 末尾，此时 data_dynamic 变成 11 个通道！
            data_dynamic = np.concatenate([data_dynamic, vpd_scaled], axis=0)
        # ==========================================
