...
def parse_args():
    ...
    parser.add_argument('--test_years', nargs='+', type=int, default=[2020, 2021], 
                        help='test years [default: 2020 2021]')
    ...
    parser.add_argument('--model_dir',
                        type=str, default=r'./log/test_SwinGCA_2d3d_ws_2_vpd_uv_55/test/model_checkpoints/best_loss_model.pth', help='pretrained model')
...
def test(args):
    ...
    # dataloader
    utils.log_string(logger, f"loading dataset for years {args.test_years}...")

    # 用一个列表来收集每年生成的数据集
    datasets_list = []
    
    for year in args.test_years:
        ds = FireCubeLoader(root=args.data_dir, mode='val', is_scale=args.is_scale, neg_pos_ratio=args.neg_pos_ratio_val,
                            val_year=year, negative=args.negative, nan_fill=args.nan_fill,
    ...
    # 将多个年份的数据集无缝拼接到一起！
    VAL_DATASET = ConcatDataset(datasets_list)
