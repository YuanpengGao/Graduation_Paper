...
from FireCube_dataloader_vpd import FireCubeLoader
...
def parse_args():
    ...
    parser.add_argument('--is_aug', type=bool, default=True, help='data augmentation [default: False]')
    ...
    parser.add_argument('--optimizer', type=str, default='AdamW', help='Adam [default: Adam]')
    parser.add_argument('--weight_decay', type=float, default=0.03, help='weight decay [default: 2e-2]')

    parser.add_argument('--drop_out', type=float, default=0.3, help='dropout ratio [default: 0.5]')
    ...
    parser.add_argument('--lr_step_size', type=int, default=12, help='learning rate step decay')
...
def train(args):
    ...
    experiment_name = f"{args.model}_{args.n_epochs}epoch_{timestamp}"
    swanlab_log_dir = os.path.join(args.log_dir,'swanlab')

    swanlab.init(
        project="Graduation_Paper",  # 项目名称，你可以自己起个高大上的名字
        name=experiment_name,        # 本次实验的名称 (比如 CNN-baseline, Swin-Dual)
        config=vars(args),              # 自动记录所有 argparse 的超参数！
        logdir=swanlab_log_dir            # 日志保存路径
    )
    ...
    if args.model == 'CNN':
        ...
        if torch.cuda.device_count() > 1:
            print(f"Let's use {torch.cuda.device_count()} GPUs!")
            classifier = torch.nn.DataParallel(classifier)
    elif args.model == 'DualBranchSwinGCA':
        # 完美复用你传给 CNN 的参数逻辑
        classifier = import_class(args.model)(
            input_channels_d=len(args.dynamic_features)+1,
            input_channels_s=len(args.static_features),
            input_channels_c=len(args.clc_features),
            n_classes=2,
            drop_out=args.drop_out,  # 会被 **kwargs 安全吸收
            pe=args.PE,              # 会被 **kwargs 安全吸收
            device=device            # 会被 **kwargs 安全吸收
        ).to(device)
        
        if torch.cuda.device_count() > 1:
            print(f"Let's use {torch.cuda.device_count()} GPUs!")
            classifier = torch.nn.DataParallel(classifier)
    elif args.model == 'DualBranchSwin2d3d':
        # 完美复用你传给 CNN 的参数逻辑
        classifier = import_class(args.model)(
            input_channels_d=len(args.dynamic_features)+1,
            input_channels_s=len(args.static_features),
        ...
        if torch.cuda.device_count() > 1:
            print(f"Let's use {torch.cuda.device_count()} GPUs!")
            classifier = torch.nn.DataParallel(classifier)
    elif args.model == 'DualBranchSwinAdaLN':
        # 完美复用你传给 CNN 的参数逻辑
        classifier = import_class(args.model)(
    ...
    for epoch in range(args.n_epochs):
        ...
        val_loss_scalar = mean_loss_val.item() if torch.is_tensor(mean_loss_val) else mean_loss_val

        # 构建详细的监控字典
        swanlab.log({
            # ================= 1. 损失函数 (Loss) =================
            "Train/Loss": mean_loss_train,
            "Train/Precision": eval_train.precision[1],
            "Train/Recall": eval_train.accuracy[1],
            "Train/F1-Score": eval_train.F1[1],
            "Train/AUROC": eval_train.AUROC,
            "Train/OA": eval_train.accuracy_all,

            "Val/Loss": mean_loss_val,
            "Val/Precision": eval_val.precision[1],
            "Val/Recall": eval_val.accuracy[1],
            "Val/F1-Score": eval_val.F1[1],
            "Val/AUROC": eval_val.AUROC,
            "Val/OA": eval_val.accuracy_all,
            "Val/IoU": eval_val.iou[1],
            
            
            # ================= 4. 负样本：背景 (Class 0 - Background) =================
            "Val_Background/Precision": eval_val.precision[0],
            "Val_Background/Recall": eval_val.accuracy[0],
            "Val_Background/F1-Score": eval_val.F1[0],
            "Val_Background/IoU": eval_val.iou[0],
